...
/* Built-in engine types temporarily shadowed by Python presenters. */
static ListBase R_engines_shadowed = {nullptr, nullptr};
#endif
...
#if defined(__EMSCRIPTEN__) && defined(WITH_HEADLESS)
  RenderEngineType *shadowed = static_cast<RenderEngineType *>(
      BLI_findstring(&R_engines_shadowed, et->idname, offsetof(RenderEngineType, idname)));
#endif
  BLI_freelinkN(&R_engines, et);
#if defined(__EMSCRIPTEN__) && defined(WITH_HEADLESS)
  if (shadowed) {
    BLI_remlink(&R_engines_shadowed, shadowed);
    BLI_addtail(&R_engines, shadowed);
  }
#endif
  return true;
}
...
#if defined(__EMSCRIPTEN__) && defined(WITH_HEADLESS)
    if (srna == nullptr) {
      BLI_remlink(&R_engines, et);
      BLI_addtail(&R_engines_shadowed, et);
    }
    else {
      if (!rna_RenderEngine_unregister(bmain, srna)) {
        BKE_reportf(reports,
                    RPT_ERROR,
      ...
      /* Unregister restores a shadowed built-in. Keep it shadowed while its Python replacement
       * is re-registered, so the active list never contains duplicate IDs. */
      RenderEngineType *restored = static_cast<RenderEngineType *>(
          BLI_findstring(&R_engines, dummy_et.idname, offsetof(RenderEngineType, idname)));
      if (restored && restored->rna_ext.srna == nullptr) {
        BLI_remlink(&R_engines, restored);
        BLI_addtail(&R_engines_shadowed, restored);
      }
    }
#else
