...
AssetMetaData *blo_bhead_id_asset_data_address(const FileData *fd, const BHead *bhead)
{
  ...
  if (!(fd->flags & FD_FLAGS_FILE_POINTSIZE_IS_4)) {
    /* The lightweight asset reader must use the full reader's address mapping. */
    uint64_t file_address;
    memcpy(&file_address,
           POINTER_OFFSET(bhead, sizeof(*bhead) + fd->id_asset_data_offset),
           sizeof(file_address));
    return reinterpret_cast<AssetMetaData *>(uintptr_t(uint32_from_uint64_ptr(file_address)));
  }
#endif
...
#ifdef __EMSCRIPTEN__
  if (!(fd->flags & (FD_FLAGS_FILE_POINTSIZE_IS_4 | FD_FLAGS_IS_MEMFILE))) {
    ...
    for (BHead *block = bhead; block; block = blo_bhead_next(fd, block)) {
      ...
      if (block->old == nullptr || addresses.add(block->old)) {
        continue;
      }
      BLO_reportf_wrap(fd->reports,
                       RPT_ERROR,
                       "Cannot open this 64-bit .blend on 32-bit WebAssembly: distinct DATA "
                       "blocks share address %p after pointer narrowing. Reading this file "
                       "would corrupt its data; use a 64-bit Blender build.",
                       block->old);
      return nullptr;
    }
  }
