                                     ...
                                     const bool use_grease_pencil)
{
  ...
  if (use_engine) {
    ...
    /* A headless build with GPU_BACKEND_NONE has no context to activate. Without this guard,
     * background grease-pencil or compositor setup reaches WM_system_gpu_context_activate with
     * a null system context. */
    else if (G.background && GPU_backend_get_type() != GPU_BACKEND_NONE &&
             ((engine->has_grease_pencil && use_grease_pencil) ||
