...
/* Emscripten aligns this aggregate to 8 bytes on wasm32, while Scene's 32-bit DNA layout places
 * CustomData_MeshMasks at a 4-byte boundary. The members are already naturally placed at 8-byte
 * offsets, so packing only lowers the aggregate alignment without changing its data. */
