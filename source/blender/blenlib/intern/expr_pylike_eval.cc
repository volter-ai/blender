...
enum eExprSoftwareFloatError {
  EXPR_FLOAT_DIV_BY_ZERO = 1 << 0,
  EXPR_FLOAT_INVALID = 1 << 1,
};

#ifdef __EMSCRIPTEN__
static thread_local int expr_float_errors;

static void expr_clear_float_errors()
{
  expr_float_errors = 0;
}

static int expr_get_float_errors()
{
  return expr_float_errors;
}

static void expr_flag_div_by_zero()
{
  expr_float_errors |= EXPR_FLOAT_DIV_BY_ZERO;
}

static void expr_flag_invalid()
{
  expr_float_errors |= EXPR_FLOAT_INVALID;
}
#else
static void expr_clear_float_errors()
{
...
static int expr_get_float_errors()
{
  const int flags = fetestexcept(FE_DIVBYZERO | FE_INVALID);
  return ((flags & FE_DIVBYZERO) ? EXPR_FLOAT_DIV_BY_ZERO : 0) |
         ((flags & FE_INVALID) ? EXPR_FLOAT_INVALID : 0);
}
#endif
...
#undef FAIL_IF
  ...
  *r_result = stack[0];
  ...
  const int flags = expr_get_float_errors();
  if (flags) {
    return (flags & EXPR_FLOAT_INVALID) ? EXPR_PYLIKE_MATH_ERROR : EXPR_PYLIKE_DIV_BY_ZERO;
  }
...
static double op_sin(double arg)
{
#ifdef __EMSCRIPTEN__
  if (std::isinf(arg)) {
    expr_flag_invalid();
...
static double op_cos(double arg)
{
#ifdef __EMSCRIPTEN__
  if (std::isinf(arg)) {
    expr_flag_invalid();
...
static double op_tan(double arg)
{
#ifdef __EMSCRIPTEN__
  if (std::isinf(arg)) {
    expr_flag_invalid();
...
static double op_asin(double arg)
{
#ifdef __EMSCRIPTEN__
  if (fabs(arg) > 1.0) {
    expr_flag_invalid();
...
static double op_acos(double arg)
{
#ifdef __EMSCRIPTEN__
  if (fabs(arg) > 1.0) {
    expr_flag_invalid();
...
static double op_log(double arg)
{
#ifdef __EMSCRIPTEN__
  if (arg == 0.0) {
    expr_flag_div_by_zero();
...
static double op_sqrt(double arg)
{
#ifdef __EMSCRIPTEN__
  if (arg < 0.0) {
    expr_flag_invalid();
...
static double op_pow(double a, double b)
{
#ifdef __EMSCRIPTEN__
  if (a == 0.0 && b < 0.0 && std::isfinite(b)) {
    expr_flag_div_by_zero();
  }
  else if (a < 0.0 && std::isfinite(a) && std::isfinite(b) && trunc(b) != b) {
    expr_flag_invalid();
  }
#endif
  return pow(a, b);
...
static double op_fmod(double a, double b)
{
#ifdef __EMSCRIPTEN__
  if ((b == 0.0 && !std::isnan(a)) || (std::isinf(a) && !std::isnan(b))) {
    expr_flag_invalid();
  }
#endif
  return fmod(a, b);
...
{
#ifdef __EMSCRIPTEN__
  return op_div(op_log(a), op_log(b));
#else
  return log(a) / log(b);
#endif
}
...
{
#ifdef __EMSCRIPTEN__
  return op_add(op_mul(a, op_sub(1.0, x)), op_mul(b, x));
#else
  return a * (1.0 - x) + b * x;
#endif
}
...
{
#ifdef __EMSCRIPTEN__
  double t = op_div(op_sub(x, a), op_sub(b, a));
#else
  double t = (x - a) / (b - a);
#endif
  CLAMP(t, 0.0, 1.0);
#ifdef __EMSCRIPTEN__
  return op_mul(op_mul(t, t), op_sub(3.0, op_mul(2.0, t)));
#else
...
static BuiltinOpDef builtin_ops[] = {
    ...
    {"sin", UnaryOpFunc(op_sin)},
    {"cos", UnaryOpFunc(op_cos)},
    {"tan", UnaryOpFunc(op_tan)},
    {"asin", UnaryOpFunc(op_asin)},
    {"acos", UnaryOpFunc(op_acos)},
    {"atan", UnaryOpFunc(atan)},
    {"atan2", BinaryOpFunc(atan2)},
    {"exp", UnaryOpFunc(exp)},
    {"log", UnaryOpFunc(op_log)},
    {"log", BinaryOpFunc(op_log2)},
    {"sqrt", UnaryOpFunc(op_sqrt)},
    {"pow", BinaryOpFunc(op_pow)},
    {"fmod", BinaryOpFunc(op_fmod)},
